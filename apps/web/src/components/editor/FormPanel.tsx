...
const FIELD =
  "w-full resize-none rounded-xs bg-transparent px-1 -mx-1 outline-none transition-colors " +
...
function TinyButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className="shrink-0 rounded-xs px-1 font-mono text-[0.6875rem] leading-none text-ink-faint opacity-0 transition hover:text-stamp focus-visible:opacity-100 group-hover:opacity-100"
    >
      {children}
    </button>
  );
          ...
          {doc.skills.map((group, gi) => (
            <div key={gi} className="group flex flex-wrap items-baseline gap-x-1.5 gap-y-1">
              ...
              {group.items.map((item, ii) => (
                <span
                  key={ii}
                  className="inline-flex items-center gap-0.5 rounded-xs bg-paper-sunk px-1.5 py-0.5 text-[0.8125rem]"
                >
                  {item}
                  <button
                    type="button"
