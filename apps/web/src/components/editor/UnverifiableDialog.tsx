      ...
      >
        <p
          className={`text-[0.875rem] leading-relaxed ${
            tone === "refused"
              ? "text-ink-muted line-through decoration-stamp/60"
              : tone === "missing"
  ...
  return (
    ...
    >
      <div
        ...
        onKeyDown={onKeyDown}
        className="rise sheet relative my-auto w-full max-w-[44rem] shadow-[0_24px_60px_-30px_rgba(23,20,15,0.55)]"
      >
        {/* The stamp. A document that has been checked gets a mark on it. */}
        <div aria-hidden="true" className="h-[3px] w-full bg-stamp" />

        <div className="px-6 pt-6 pb-5 sm:px-8">
          <p className="label text-stamp">Refused, not failed</p>
          <h2
            id="refusal-title"
            ref={headingRef}
            tabIndex={-1}
            className="mt-2 font-display text-[1.75rem] leading-tight text-ink outline-none"
          >
            {count === 1
              ...
              <li key={index} className="border-b border-rule px-6 py-5 sm:px-8">
                <div className="flex items-baseline gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="font-mono text-[0.6875rem] tracking-wider text-stamp">
                ...
                <div className="mt-1 pl-0 sm:pl-8">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button
                      ...
                      className="rounded-xs border border-ink bg-ink px-3 py-1.5 text-[0.8125rem] text-paper-raised transition-colors hover:bg-ink/85"
                    >
                    ...
                    <button
                      ...
                      className="rounded-xs border border-rule-strong px-3 py-1.5 text-[0.8125rem] text-ink-muted transition-colors hover:border-stamp hover:text-stamp"
                    >
                      Drop it
                    </button>
                  </div>
        ...
        <div className="flex items-center justify-between gap-4 px-6 py-4 sm:px-8">
          ...
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 rounded-xs border border-rule-strong px-3 py-1.5 text-[0.8125rem] text-ink transition-colors hover:bg-paper-sunk"
          >
            Close
          </button>
        </div>
