  ...
  return (
    <div className="flex h-full flex-col">
      <button
        ...
        className={[
          "group relative flex w-full flex-1 cursor-pointer flex-col items-center justify-center gap-3 rounded-xs border border-dashed px-6 py-10 text-center transition-colors duration-150",
          "disabled:cursor-progress",
          dragging
            ? "border-stamp bg-stamp-soft"
            : error
              ? "border-stamp/60 bg-paper-raised"
              : "border-rule-strong bg-paper-raised hover:border-ink-faint hover:bg-paper-sunk/60",
        ].join(" ")}
      >
        {/* Corner registration ticks: the panel reads as a paste-up area. */}
        <Ticks active={dragging} />
        ...
        {busy ? (
          ...
          <>
            <p className="label text-traced">File ready</p>
            ...
            <span className="mt-1 text-[0.8125rem] text-ink-muted underline decoration-rule-strong underline-offset-4">
      ...
      <div id={statusId} role="status" aria-live="polite" className="min-h-[3.25rem] pt-3">
        {error ? (
          <p className="rounded-xs border-l-2 border-stamp bg-stamp-soft/60 px-3 py-2 text-[0.8125rem] leading-relaxed text-ink">
...
function Ticks({ active }: { active: boolean }) {
  const color = active ? "bg-stamp" : "bg-rule-strong";
  return (
    <span aria-hidden="true" className="pointer-events-none absolute inset-2">
      {(
        ...
        <span key={position} className={`absolute ${position} size-2.5`}>
          <span className={`absolute inset-x-0 top-0 h-px ${color} opacity-70`} />
          <span className={`absolute inset-y-0 left-0 w-px ${color} opacity-70`} />
        </span>
      ))}
    </span>
  );
