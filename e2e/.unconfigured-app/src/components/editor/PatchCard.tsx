    ...
    <p className="text-[0.8125rem] leading-relaxed">
      {chunks.map((chunk, i) =>
        chunk.kind === "same" ? (
          ...
          <del key={i} className="bg-stamp-soft text-stamp no-underline line-through">
            {chunk.text}
          </del>
        ) : (
...
function OpRow({ doc, op, showChanges }: { doc: ResumeDoc; op: Op; showChanges: boolean }) {
  ...
  return (
    <li className="border-t border-rule px-3 py-2.5 first:border-t-0">
      ...
      {op.op === "remove" ? (
        <p className="mt-1 text-[0.8125rem] leading-relaxed text-stamp line-through">{before}</p>
...
export function PatchCard({
  ...
  return (
    <article
      ...
      className="rise sheet border-l-2 border-l-stamp"
    >
      <header className="flex items-start justify-between gap-3 px-3 py-2.5">
        ...
        <span
          ...
          className={`shrink-0 rounded-xs border px-1.5 py-0.5 font-mono text-[0.6875rem] tabular-nums ${
            delta > 0
      ...
      <footer className="flex flex-wrap items-center gap-2 border-t border-rule px-3 py-2.5">
        <button
          ...
          className="rounded-xs border border-ink bg-ink px-3 py-1 text-[0.8125rem] text-paper-raised transition-colors hover:bg-ink/85"
        >
          Accept
        </button>
        <button
          ...
          className="rounded-xs border border-rule-strong px-3 py-1 text-[0.8125rem] text-ink-muted transition-colors hover:border-stamp hover:text-stamp"
        >
          Decline
        </button>
        <button
