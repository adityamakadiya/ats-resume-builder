...
export function UserMenu({ email }: UserMenuProps) {
  ...
  return (
    <div className="w-full">
      <DropdownMenu>
        <DropdownMenuTrigger
          className="flex w-full items-center gap-2.5 rounded-md px-2 py-2 text-left transition-colors hover:bg-paper-sunk aria-expanded:bg-paper-sunk"
          aria-label={email ? `Account menu for ${email}` : "Account menu"}
        >
          <span
            aria-hidden="true"
            className="grid size-8 shrink-0 place-items-center rounded-full bg-stamp-soft text-[0.6875rem] font-semibold tracking-wide text-[var(--stamp-strong)]"
          >
            {initials(email)}
          </span>
        ...
        <DropdownMenuContent
          align="start"
          side="top"
          className="min-w-56 rounded-lg border border-rule bg-paper-raised p-1 shadow-lg ring-0"
        >
          ...
          <DropdownMenuItem className="rounded-md px-2 py-1.5" disabled>
            <User2 aria-hidden="true" />
            Account settings
            <span className="ml-auto rounded-full bg-paper-sunk px-2 py-0.5 text-[0.6875rem] font-medium text-ink-muted">
              Soon
            </span>
          </DropdownMenuItem>
          <DropdownMenuItem
      ...
      {error ? (
        <p className="mt-1.5 px-2 text-[0.75rem] leading-snug text-[var(--refused)]">{error}</p>
      ) : null}
    </div>
  );
}
