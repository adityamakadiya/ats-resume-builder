 ...
 *
 * THIRD, what the thumbnails render is the candidate's own resume whenever
 * there is one to render. Someone who has just uploaded a CV and is shown a
 * stranger's has been given a preview of nothing: templates differ most in
 * how they cope with YOUR number of roles and YOUR length of bullet. When
 * there is nothing to pour in, the fixture is used and the preview says
 * "Sample content" on its face, so nobody has to guess whose resume they are
 * looking at.
 */

import { useMemo, useRef, useState } from "react";
import { AlertTriangle, Check } from "lucide-react";
import type { ResumeDoc, TemplateCategory, TemplateMeta } from "@ats/templates";
...
export type TemplatePickerProps = {
  ...
  /**
   * The candidate's own document, rebuilt from their upload. Absent means
   * there is nothing to pour in, and the previews fall back to the fixture
...
export function TemplatePicker({
  ...
  const preview = doc ?? rich;
  const isSample = doc === undefined;
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_23rem] lg:gap-10 xl:grid-cols-[minmax(0,1fr)_27rem]">
      <div>
        ...
        {/* ------------------------------------------------------- grid -- */}
        {visible.length === 0 ? (
          <p className="mt-8 rounded-lg border border-dashed border-rule-strong px-4 py-8 text-center text-[0.875rem] text-ink-muted">
      ...
      <aside className="lg:sticky lg:top-8 lg:self-start" aria-label="Template preview">
        <div className="overflow-hidden rounded-xl border border-rule bg-paper-raised">
          <div className="border-b border-rule px-5 py-4">
            ...
            <h2 className="mt-1.5 font-display text-2xl leading-none font-semibold tracking-[-0.01em] text-ink">
              {selected.name}
            </h2>
          ...
          <div className="relative flex justify-center bg-paper-sunk px-5 py-5">
            <div className="rounded-sm border border-rule-strong shadow-[0_2px_10px_-4px_rgba(15,23,42,0.25)]">
              <TemplateThumbnail template={selected} doc={preview} width={288} />
            </div>
            {/*
              Whose words are in the preview. Nobody who uploaded a resume
              should have to work out that the page in front of them belongs
              to an invented person.
            */}
            <span
              className={[
                "absolute top-3 right-3 rounded-md border px-2 py-0.5 font-mono text-[0.625rem] tracking-[0.08em] uppercase",
                isSample
                  ? "border-caution/40 bg-caution-soft text-caution"
                  : "border-traced/40 bg-traced-soft text-traced",
              ].join(" ")}
            >
              {isSample ? "Sample content" : "Your resume"}
            </span>
          </div>
          ...
          {selected.warning ? (
            ...
            <p className="m-5 mb-0 rounded-lg border-l-2 border-traced bg-traced-soft/50 px-3 py-2.5 text-[0.8125rem] leading-relaxed text-ink">
          ...
          <div className="p-5">
            {error ? (
              <p
                role="alert"
                className="mb-3 rounded-lg border-l-2 border-stamp bg-stamp-soft/60 px-3 py-2 text-[0.8125rem] leading-relaxed text-ink"
              >
            ...
            <Button
              ...
              className="w-full rounded-md bg-stamp py-5 text-[0.9375rem] font-medium text-paper-raised hover:bg-[color:var(--stamp-strong)]"
            >
...
export function TwoColumnWarning({
  ...
  return (
    <p
      className={`flex gap-2.5 rounded-lg border-l-2 border-caution bg-caution-soft/70 px-3 py-2.5 text-[0.8125rem] leading-relaxed text-ink ${
        className ?? ""
      }`}
    >
    ...
    >
      <span className="relative flex justify-center overflow-hidden bg-paper-sunk px-4 pt-4">
        <span className="block border border-rule-strong">
          ...
          <TemplateThumbnail template={template} doc={doc} width={208} reveal={0.62} />
        </span>
        <span
          aria-hidden="true"
